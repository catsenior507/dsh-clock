window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var clock_module_css_default = {
			...
			"launcherBadge": "_8SpRlq_launcherBadge",
			"listWrap": "_8SpRlq_listWrap",
		...
		*/
		...
		function AlarmRow(props) {
			...
			}, "▶") : null, alarm.status === "pending" ? react.default.createElement("button", {
				className: clock_module_css_default.iconButton,
				title: "取消",
				onClick: () => onCancel(alarm.id)
			}, "⊘") : null, react.default.createElement("button", {
				className: clock_module_css_default.iconButton,
				title: "删除",
		...
		function ClockApp() {
			...
			const submit = (0, react.useCallback)(async () => {
				...
				try {
					await call("/alarms", {
						at: instantFromLocal(selectedDay, time),
						timeZone: browserZone(),
						keyword: keyword.trim(),
						note: note.trim() === "" ? void 0 : note.trim(),
						label: void 0,
						sessionId,
						sessionTitle: state === null ? void 0 : state.sessions.find((row) => row.id === sessionId)?.title ?? void 0,
						origin: "user"
					});
					setKeyword("");
					setNote("");
			...
			return react.default.createElement(react.default.Fragment, null, react.default.createElement("button", {
				className: clock_module_css_default.launcher,
				style: {
					left: position.x,
					top: position.y
				},
			...
			}, react.default.createElement("span", { style: { fontSize: "18px" } }, "⏰"), pendingCount > 0 ? react.default.createElement("span", { className: clock_module_css_default.launcherBadge }, String(pendingCount)) : null), !open ? null : react.default.createElement("div", {
				className: clock_module_css_default.panel,
				style: {
					left: panelLeft,
					bottom: panelBottom
				}
			}, react.default.createElement("div", { className: clock_module_css_default.header }, react.default.createElement("span", { className: clock_module_css_default.title }, "时钟与日历"), react.default.createElement("span", { className: clock_module_css_default.headerSpacer }), react.default.createElement("button", {
				className: clock_module_css_default.iconButton,
				onClick: () => setOpen(false),
				title: "关闭"
			}, "✕")), react.default.createElement("div", { className: clock_module_css_default.clock }, react.default.createElement("div", { className: clock_module_css_default.clockTime }, formatTime(now, zone)), react.default.createElement("div", { className: clock_module_css_default.clockMeta }, react.default.createElement("div", { className: clock_module_css_default.clockDate }, formatDate(now, zone)), react.default.createElement("div", { className: clock_module_css_default.clockZone }, zone)), react.default.createElement("div", { className: clock_module_css_default.nextAlarm }, react.default.createElement("div", { className: clock_module_css_default.nextLabel }, "下一个唤醒"), nextPending === null ? react.default.createElement("div", { className: clock_module_css_default.nextValueIdle }, "无") : react.default.createElement("div", { className: clock_module_css_default.nextValue }, countdownText(nextPending.at - now)))), react.default.createElement("div", { className: clock_module_css_default.calendar }, react.default.createElement("div", { className: clock_module_css_default.calendarBar }, react.default.createElement("button", {
				className: clock_module_css_default.iconButton,
			...
			}))), react.default.createElement("div", { className: clock_module_css_default.listWrap }, alarms.length === 0 ? react.default.createElement("div", { className: clock_module_css_default.empty }, "还没有任何唤醒。选一个日期和时间，填上关键词即可。") : alarms.map((alarm) => react.default.createElement(AlarmRow, {
				key: alarm.id,
				alarm,
				now,
				onFire: (id) => {
			...
			}))), react.default.createElement("div", { className: clock_module_css_default.form }, react.default.createElement("div", { className: clock_module_css_default.formRow }, react.default.createElement("input", {
				className: clock_module_css_default.input + " " + clock_module_css_default.inputTime,
				type: "time",
				value: time,
			...
			})), react.default.createElement("div", { className: clock_module_css_default.formRow }, react.default.createElement("select", {
				className: clock_module_css_default.select,
				value: sessionId,
				onChange: (event) => setSessionId(event.target.value)
			}, react.default.createElement("option", { value: "" }, "选择要唤醒的对话…"), (state === null ? [] : state.sessions).map((row) => react.default.createElement("option", {
				key: row.id,
				value: row.id
			}, (row.cold ? "（未打开）" : "") + (row.title === "" ? row.id : row.title))))), react.default.createElement("div", { className: clock_module_css_default.formRow }, react.default.createElement("input", {
				className: clock_module_css_default.input,
				placeholder: "备注（可选）：唤醒时要做什么",
				value: note,
				onChange: (event) => setNote(event.target.value)
			})), react.default.createElement("div", { className: clock_module_css_default.formRow }, react.default.createElement("button", {
				className: clock_module_css_default.button,
				disabled: busy,
				onClick: () => {
					submit();
				}
			}, "在 " + selectedDay + " " + time + " 唤醒"), react.default.createElement("span", { className: clock_module_css_default.headerSpacer }), react.default.createElement("button", {
				className: clock_module_css_default.iconButton,
				title: "刷新",
				onClick: () => {
					refresh();
				}
			}, "⟳")), state !== null && state.scheduler.systemScheduler ? react.default.createElement("div", { className: clock_module_css_default.hint }, "系统计划任务：" + (state.scheduler.systemTaskArmedFor === null ? "未挂载" : formatDate(state.scheduler.systemTaskArmedFor, zone) + " " + formatHm(state.scheduler.systemTaskArmedFor, zone))) : react.default.createElement("div", { className: clock_module_css_default.hint }, "系统计划任务不可用，仅使用进程内定时器"), error === "" ? null : react.default.createElement("div", { className: clock_module_css_default.error }, error))));
		}
		//#endregion
		...
		*/
		function apply(ctx) {
			...
			try {
				...
				mount.remove();
				throw new Error("[clock] mounting the calendar panel failed: React root creation threw");
			}
			ctx.effect(() => () => {
				root?.unmount();
				mount.remove();
