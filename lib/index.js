		...
		try {
			...
			for (const snapshot of snapshots) {
				...
				try {
					...
					const events = normalizeEvents((await handle.read()).events ?? []);
...
function apply(ctx, config) {
	...
	service.note("apply: sessionController=" + (controller === void 0 ? "missing" : "present") + " tool=" + tool + " api=" + api + " systemScheduler=" + String(service.config.useSystemScheduler && systemSchedulerSupported()));
}
/**
