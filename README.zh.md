...
- **分支会被发现**：分支一个对话复制的是对话，不是闹钟表 —— 分支之前设的闹钟会继续指向原来那条，
  于是分支**永远不会被唤醒**。面板会为每个「尚未覆盖」的分支问一次：把闹钟复制到这个分支，或者拒绝；
  两种回答都会被记住，不会再问。
- **闹钟列表**：分成「**待触发**」和「**已结束**」两栏，各自是一个独立的滚动窗口，所以历史再长也不会把面板撑长；
  待触发按时间正序、已结束按时间倒序。每行可以**编辑**、立即触发、取消、删除；点「编辑」会把这条闹钟装回表单
  （时间、关键词、内容、目标对话都能改），按钮变成「**保存修改**」。**只有待触发的能编辑** —— 改一条已经触发过的，
  等于承诺一次不会发生的投递。已触发的行会写明是准点还是迟到，投递失败的原因也留在行上，不会被吞掉。
...
插件还带一个名为 `clock` 的 **skill**。工具只靠一行描述被广告出去；而**从没见过这个插件的对话**，
是靠这个 skill 才知道那一行装不下的东西：目标可以是一个**已关闭**的对话、被唤醒的对话是**原地继续而不是
fork**、以及迟到投递会在消息里写明迟了多久。它以 `bundled` 来源注册，所以在这个 profile 的**每个对话**
的技能目录里都能看到。
